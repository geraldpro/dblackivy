...
if(isset($_FILES['imagefiles']['name'])&& !empty($_FILES['imagefiles']['name'])&& !empty($_FILES['imagefiles']['size'])){
	 if( is_uploaded_file($_FILES['imagefiles']['tmp_name'])){
		...
		if(move_uploaded_file($tmp_name,$img_dir.$_FILES['imagefiles']['name'])){
			$stmt=$mdb->mysqli->prepare("UPDATE banner SET bannername=?,bannerphoto=? WHERE bannerid =?");
            $stmt-> bind_param('ssd',$bannername,$bannerphoto,$bannerid);
...
if(is_numeric($id)){
$count=0;$count++;
$stmt=$mdb->mysqli->query("SELECT bannerid,bannername,bannerphoto FROM banner WHERE bannerid = $id",MYSQLI_USE_RESULT);
    ...
    <div class="container-fluid">
		<div class="row">
			...
			<div class="col-sm-9 col-sm-offset-3 col-md-10 col-md-offset-2 main">
			  <h1 class="page-header">Edit Banner Photo</h1>
               <div class="row">
               <div class="col-md-12">
               <div class="form-group col-md-8">
			<form role="form" action="" method="post" enctype="multipart/form-data"/>
             <input name="bannerid" type="hidden" value="<?php echo $bannerid ; ?>">

               <label for="Advertname">Banner Name</label>
<input type="text"  name="bannername" value="<?php if(isset($_GET['id'])){echo $bannername;} ?>" class="form-control" placeholder="Enter Banner Name">
								</div>
                                
                        <div class="col-sm-12">
										<div class="col-sm-4 col-md-2">
											<div class="thumbnail" style="text-align:center;">
<a href="#"><?php if(isset($_GET['id'])){echo "<img src=http://localhost/dblackivy/banner_images/".$bannerphoto."> <br>";} ?></a>		 
