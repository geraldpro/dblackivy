...
$stmt = $mdb->mysqli->query('SELECT postid,postcategory,postitle,postbody,postphoto FROM postnews WHERE postcategory="Fashion" ORDER BY postid DESC LIMIT 6', MYSQLI_USE_RESULT);
$count4= 0;
while($data=$stmt->fetch_array(MYSQLI_ASSOC)){
	   $dataid[] = $data['postid'];
	   $datacategory[] = $data['postcategory'];
...
<h4 style="margin-top:-10px;"><a href="read-more.php?id=<?php echo $dataid[$i] ;?>"><?php echo strtoupper($datatitle[$i]);?></a>        <p style="padding-right:5px;"><?php echo substr($datacontent[$i],0,60);?>....</p>
