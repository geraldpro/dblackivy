...
$stmt = $mdb->mysqli->query('SELECT postid,postcategory,postitle,postbody,postphoto FROM postnews WHERE postcategory="Inspirational" ORDER BY postid DESC LIMIT 6', MYSQLI_USE_RESULT);
$count4= 0;
while($data=$stmt->fetch_array(MYSQLI_ASSOC)){
	   $dataid[] = $data['postid'];
	   $datacategory[] = $data['postcategory'];
