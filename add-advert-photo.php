...
include_once "create-advert.class.php";

$mdb=new connect;
$create = new create_advert;

if(isset($_POST['submit'])){
  $_POST = array_map('stripslashes',$_POST);
   extract ($_POST);
	
	$advertdate = date("Y-m-d");
...
if(isset($_FILES['imagefiles']['name'])&& !empty($_FILES['imagefiles']['name'])&& !empty($_FILES['imagefiles']['size'])){
	 if( is_uploaded_file($_FILES['imagefiles']['tmp_name'])){
		...
		if(move_uploaded_file($tmp_name,$img_dir.$_FILES['imagefiles']['name'])){
		  ...
		  $create->insertparameters($advertname,$advertphoto,$advertdate);
...
<html lang="en">
  ...
  <title>Add-advert-photo -- Black Ivy</title>
  <!-- Bootstrap core CSS -->
...
<input type="text"  name="advertname" value="<?php if(isset($error)){echo $_POST['advertname'];}?>" class="form-control" placeholder="Enter Advert Name">
								</div>
					<div class="form-group col-md-8">
