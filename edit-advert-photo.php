...
if(isset($_FILES['imagefiles']['name'])&& !empty($_FILES['imagefiles']['name'])&& !empty($_FILES['imagefiles']['size'])){
	 if( is_uploaded_file($_FILES['imagefiles']['tmp_name'])){
		 ...
		 $advertphoto = $_FILES['imagefiles']['name'];
         ...
         $img_dir = $_SERVER['DOCUMENT_ROOT']."/dblackivy/advert_images/" ;
        
		if(move_uploaded_file($tmp_name,$img_dir.$_FILES['imagefiles']['name'])){
			$stmt=$mdb->mysqli->prepare("UPDATE advert SET advertname=?,advertphoto=? WHERE advertid =?");
            $stmt-> bind_param('ssd',$advertname,$advertphoto,$advertid);
...
if(is_numeric($id)){
$count=0;$count++;
$stmt=$mdb->mysqli->query("SELECT advertid,advertname,advertphoto FROM advert WHERE advertid = $id",MYSQLI_USE_RESULT);
    ...
    <div class="container-fluid">
		<div class="row">
			...
			<form role="form" action="" method="post" enctype="multipart/form-data"/>
             <input name="advertid" type="hidden" value="<?php echo $advertid ; ?>">

               <label for="Advertname">Advert Name</label>
<input type="text"  name="advertname" value="<?php if(isset($_GET['id'])){echo $advertname;} ?>" class="form-control" placeholder="Enter Advert Name">
								</div>
                                
                        <div class="col-sm-12">
										<div class="col-sm-4 col-md-2">
											<div class="thumbnail" style="text-align:center;">
<a href="#"><?php if(isset($_GET['id'])){echo "<img src=http://localhost/dblackivy/advert_images/".$advertphoto."> <br>";} ?></a>		 
